...
渐进式rehash让字典同时持有ht\[0]和ht\[1]，通过rehashidx来记录rehash的进行，rehash完成之后置-1
#### 要点总结   
...
* 每一层都是一个有序饿链表
* 最底层包含所有的元素
* 一个元素在level i 的链表中，它肯定会在i层之下的链表中存在
...
#### AOF重写
#### 要点总结
* AOF文件通过保存所有修改数据库的写命令请求来保存服务器的数据库状态
* AOF文件中的所有命令都以Redis命令请求协议的额格式保存
* 命令请求会先保存到AOF缓冲区zoo那个，之后再定期写入同步到AOF文件中
* appendfsync选项的不同值与AOF持久化功能的安全性以及Redis服务器的性能有很大影响
* 服务器只要载入并重新执行保存在AOF文件中的命令就可以还原数据库状态
* AOF重写会产生一个新的AOF文件，和原有文件保存的数据库状态相同，体积更小
* AOF重写不是对原有AOF文件操作，而是遍历数据库中的键值对来实现简化语句缩小体积
* 在执行BGREWRITEAOF命令时，Redis会维护一个AOF重写缓冲区，会在子进程创建新的AOF文件时，记录服务器执行的所有写命令。子进程完成创建新的AOF文件，缓冲区内容会被追加到新的AOF文件末尾，保存新旧AOF文件数据库状态一致，最后服务器新文件改名原子覆盖远文件。
...
* 主向从传播命令来更新从服务器的状态，从要向主发送命令进行心跳检测和命令丢失检测
